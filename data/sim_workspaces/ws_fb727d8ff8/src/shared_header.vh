...
// cgra_top.v
// ============================================================================
// Top-level 2x2 Coarse-Grained Reconfigurable Array.
//
// Integrates:
//   * 4 config_mem instances  — one per PE, holding that PE's control word.
//   * 1 pe_grid instance       — the 2x2 PE mesh with nearest-neighbor routing.
//   * 1 io_pad instance        — registered boundary I/O pads.
//
// A small control FSM (IDLE -> LOAD -> RUN) sequences configuration loading
// and steady-state execution.  During LOAD, external writes program the four
// config memories (cfg_pe_sel selects which PE's memory is written).  During
// RUN, the addressed config entry of each PE is read and broadcast to its PE.
//
// All inter-PE and pad paths are registered, so there are no combinational
// loops across the array.
// ============================================================================

`include "cgra_params.vh"
